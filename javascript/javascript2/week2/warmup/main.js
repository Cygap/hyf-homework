...
display.innerText = `For the array: ${numbers}`;
const renderedResult = document.createElement("div");
renderedResult.innerText = `The new array with doubled odd numbers is "${doubleOdd(
  numbers
)}"`;
...
/**
 *
 * @param {array} numbersArray of numbers
 * @returns mapped array with the odd numbers of initial array doubled and even numbers omitted.
 */
 ...
 */
const shortMovies = movies.filter((movie) => !movie.title.includes(" "));
section.append(secondDisplay);

/**Create an array of movie titles with long movie titles */
const longMovies = movies.filter((movie) => movie.title.includes(" "));

/**
 * Count the number of movies made between 1980-1989 (including both the years)
 */
const thirdDisplay = document.createElement("div");
section.append(thirdDisplay);
secondDisplay.innerText = `The number of movies made between 1980-1989 (including both the years): ${
 ...
 */
let regex = /(Surfer)|(Alien)|(Benjamin)/i;
const countMovies = movies.filter((movie) => regex.test(movie.title));
thirdDisplay.innerText = `the total number of movies containing any of following keywords: Surfer, Alien or Benjamin (case insensitive) is: ${countMovies.length}`;
 ...
 */
regex = /(\b\w+\b).*\b\1\b/i;
const fourthDisplay = document.createElement("div");
const doubleWordsTitles = movies.filter((movie) => regex.test(movie.title));
section.append(fourthDisplay);
 ...
 */
const totalNumberOfTaggedMovies = taggedMovies.reduce(
  (result, movie) => {
    result[movie.tag] += 1;
    return result;
  },
...
fourthDisplay.innerText = `The average movie rating is ${averageMovieRating}\nthe total number of Good, Average and Bad movies is: ${JSON.stringify(
  totalNumberOfTaggedMovies
)}`;
